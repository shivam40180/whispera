...
app.post('/settings/profile-pic', auth, uploadProfile.single('photo'), async (req, res) => {
  try {
    const url = `${BASE}:${process.env.PORT || 5001}/uploads/profiles/${req.file.filename}`;
...
app.post('/upload/chat', auth, uploadChat.single('file'), async (req, res) => {
  try {
    ...
    const url = `${BASE}:${process.env.PORT || 5001}/uploads/chat/${req.file.filename}`;
...
app.post('/status', auth, uploadStatus.single('file'), async (req, res) => {
  try {
    ...
    if (req.file) statusData.fileUrl = `${BASE}:${process.env.PORT || 5001}/uploads/status/${req.file.filename}`;
